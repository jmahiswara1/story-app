...
export async function AddStoryView({ mainRoot }) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (file.size > 1024 * 1024)
      return showToast("Ukuran foto melebihi 1MB", "error");
      ...
      if (isOnline) {
        try {
          ...
          try {
            ...
            showToast(
              "Cerita disimpan offline, akan disinkronkan saat online",
              "info"
            );
            cleanupAndGoHome();
          } catch (errInner) {
            console.error("Error saving offline after API failure:", errInner);
    ...
    observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        for (const node of mutation.removedNodes) {
          if (
            node === card ||
            (node.nodeType === 1 && node.contains && node.contains(card))
          ) {
            cleanup();
            return;
          }
