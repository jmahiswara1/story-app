...
export async function HomeView({ mainRoot }) {
  ...
  async function requestNotificationPermission() {
    ...
    if (Notification.permission === 'denied') throw new Error('Izin notifikasi ditolak. Silakan aktifkan di pengaturan browser.');

    const permission = await Notification.requestPermission();
    if (permission !== 'granted') throw new Error('Izin notifikasi diperlukan untuk mengaktifkan fitur ini.');
    return true;
  }
  ...
  async function subscribePush() {
    ...
    try {
      ...
      if (!json.endpoint || !json.keys) throw new Error('Subscription data tidak valid');
      await StoryApi.subscribePush({ endpoint: json.endpoint, keys: json.keys });
      showToast('Notifikasi diaktifkan', 'success');
    } catch (error) {
      console.error('Subscribe error:', error);
      if (error.name === 'NotAllowedError') throw new Error('Izin notifikasi ditolak');
      if (error.name === 'NotSupportedError') throw new Error('Push notification tidak didukung');
  ...
  const stamenTerrain = L.tileLayer('https://stamen-tiles.a.ssl.fastly.net/terrain/{z}/{x}/{y}.jpg', { attribution: 'Map tiles by Stamen Design' });
  const stamenWatercolor = L.tileLayer('https://stamen-tiles.a.ssl.fastly.net/watercolor/{z}/{x}/{y}.jpg', { attribution: 'Map tiles by Stamen Design' });
  L.control.layers({ OSM: osm, Terrain: stamenTerrain, Watercolor: stamenWatercolor }, {}, { position: 'topright' }).addTo(map);
