...
async function http(path, { method = 'GET', headers = {}, body, auth = false } = {}) {
...
export const StoryApi = {
  async register({ name, email, password }) {
    return http('/register', { method: 'POST', body: JSON.stringify({ name, email, password }) });
  },
  async login({ email, password }) {
    const data = await http('/login', { method: 'POST', body: JSON.stringify({ email, password }) });
  ...
  async subscribePush({ endpoint, keys }) {
    return http('/notifications/subscribe', { method: 'POST', auth: true, body: JSON.stringify({ endpoint, keys }) });
  },
  async unsubscribePush({ endpoint }) {
    return http('/notifications/subscribe', { method: 'DELETE', auth: true, body: JSON.stringify({ endpoint }) });
  },
};
